...
- **`/funnel-report`** generates a self-contained Sankey diagram of your application funnel — where applications end up (ghosted, rejected, hired, still active) and, for rejections, how far they got first. Built from the tracker and `outcome.md` archive, rendered as inline SVG, fully offline. Re-run it any time after `/outcome` adds new entries.
...
│   │   ├── funnel-report.md           # /funnel-report generate application funnel Sankey diagram
...
├── reports/                           # /funnel-report output (generated HTML dashboards)
